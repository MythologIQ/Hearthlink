/**
 * UI tests for Button React component
...
test.describe('Button Component', () => {
    ...
    test('should render Button component', async ({ page }) => {
        ...
        const componentExists = await page.locator('text=Button').isVisible()
            .catch(() => false);
        
        if (!componentExists) {
            // Component may not be visible on main page - that's okay
            console.log('Button not visible on main page - checking for existence');
            
            // Check if component exists in DOM (even if not visible)
            const componentInDOM = await page.evaluate(() => {
                return document.querySelector('[class*="button"], [data-component="button"]') !== null;
            });
