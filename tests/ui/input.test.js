/**
 * UI tests for Input React component
...
test.describe('Input Component', () => {
    ...
    test('should render Input component', async ({ page }) => {
        ...
        const componentExists = await page.locator('text=Input').isVisible()
            .catch(() => false);
        
        if (!componentExists) {
            // Component may not be visible on main page - that's okay
            console.log('Input not visible on main page - checking for existence');
            
            // Check if component exists in DOM (even if not visible)
            const componentInDOM = await page.evaluate(() => {
                return document.querySelector('[class*="input"], [data-component="input"]') !== null;
            });
