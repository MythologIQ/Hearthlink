/**
 * UI tests for TaskEditor React component
...
test.describe('TaskEditor Component', () => {
    ...
    test('should render TaskEditor component', async ({ page }) => {
        // Look for component-specific elements
        const componentExists = await page.locator('text=TaskEditor').isVisible()
            .catch(() => false);
        
        if (!componentExists) {
            // Component may not be visible on main page - that's okay
            console.log('TaskEditor not visible on main page - checking for existence');
            
            // Check if component exists in DOM (even if not visible)
            const componentInDOM = await page.evaluate(() => {
                return document.querySelector('[class*="taskeditor"], [data-component="taskeditor"]') !== null;
            });
