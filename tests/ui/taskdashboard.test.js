/**
 * UI tests for TaskDashboard React component
...
test.describe('TaskDashboard Component', () => {
    ...
    test('should render TaskDashboard component', async ({ page }) => {
        // Look for component-specific elements
        const componentExists = await page.locator('text=TaskDashboard').isVisible()
            .catch(() => false);
        
        if (!componentExists) {
            // Component may not be visible on main page - that's okay
            console.log('TaskDashboard not visible on main page - checking for existence');
            
            // Check if component exists in DOM (even if not visible)
            const componentInDOM = await page.evaluate(() => {
                return document.querySelector('[class*="taskdashboard"], [data-component="taskdashboard"]') !== null;
            });
