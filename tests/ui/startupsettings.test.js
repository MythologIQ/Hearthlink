/**
 * UI tests for StartupSettings React component
 * Generated by SPEC-3 Week 2 Test Suite Expander
 */

const { test, expect } = require('@playwright/test');

test.describe('StartupSettings Component', () => {
    ...
    test('should render StartupSettings component', async ({ page }) => {
        // Look for component-specific elements
        const componentExists = await page.locator('text=StartupSettings').isVisible()
            .catch(() => false);
        
        if (!componentExists) {
            // Component may not be visible on main page - that's okay
            console.log('StartupSettings not visible on main page - checking for existence');
            
            // Check if component exists in DOM (even if not visible)
            const componentInDOM = await page.evaluate(() => {
                return document.querySelector('[class*="startupsettings"], [data-component="startupsettings"]') !== null;
            });
