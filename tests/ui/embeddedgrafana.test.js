/**
 * UI tests for EmbeddedGrafana React component
 * Generated by SPEC-3 Week 2 Test Suite Expander
 */

const { test, expect } = require('@playwright/test');

test.describe('EmbeddedGrafana Component', () => {
    ...
    test('should render EmbeddedGrafana component', async ({ page }) => {
        // Look for component-specific elements
        const componentExists = await page.locator('text=EmbeddedGrafana').isVisible()
            .catch(() => false);
        
        if (!componentExists) {
            // Component may not be visible on main page - that's okay
            console.log('EmbeddedGrafana not visible on main page - checking for existence');
            
            // Check if component exists in DOM (even if not visible)
            const componentInDOM = await page.evaluate(() => {
                return document.querySelector('[class*="embeddedgrafana"], [data-component="embeddedgrafana"]') !== null;
            });
